...
<form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	...
	<?php if($pmpro_msg) 
		{
	?>
		<div id="pmpro_message" class="alert alert-warning pmpro_message <?php echo $pmpro_msgt?>"><?php echo $pmpro_msg?></div>
	<?php
		}
		else
		{
	...
	<thead>
		<tr>
			<th>
				<?php if(count($pmpro_levels) > 1) { ?><span class="pmpro_thead-msg"><a class="btn btn-xs btn-default" href="<?php echo pmpro_url("levels"); ?>"><?php _ex('change', 'change membership level link', 'pmpro');?></a></span><?php } ?><?php _e('Membership Level', 'pmpro');?>
			</th>						
		</tr>
	</thead>
	<tbody>                
		...
		<tr id="other_discount_code_tr" style="display: none;">
			<td>
				<div>
					...
					<input class="btn btn-default" type="button" name="other_discount_code_button" id="other_discount_code_button" value="<?php _e('Apply', 'pmpro');?>" />					
				</div>				
			</td>
		</tr>
		<?php } ?>
...
<?php echo wpautop($tospage->post_content)?>
					...
					<input class="checkbox-cb" type="checkbox" name="tos" value="1" id="tos" />
					<span class="checkbox-mark"></span>
					</label>
					<label for="tos"><?php printf(__('I agree to the %s', 'pmpro'), $tospage->post_title);?></label>
				</td>
			</tr>
		</tbody>
		</table>
	...
	<tbody>                    
		<tr valign="top">		
			<td>	
				...
				<div class="pmpro-expiration-selects">
	...
	<div class="pmpro_submit">
		<?php if($pmpro_review) { ?>
			...
			<span id="pmpro_submit_span">
				...
				<input type="submit" class="btn btn-highlight pmpro_btn pmpro_btn-submit-checkout" value="<?php _e('Complete Payment', 'pmpro');?> &raquo;" />
			</span>
		...
		<?php } else { ?>
			...
			<span id="pmpro_submit_span" <?php if(($gateway == "paypalexpress" || $gateway == "paypalstandard") && $pmpro_requirebilling) { ?>style="display: none;"<?php } ?>>
				<input type="hidden" name="submit-checkout" value="1" />		
				<input type="submit" class="btn btn-highlight pmpro_btn pmpro_btn-submit-checkout" value="<?php if($pmpro_requirebilling) { if($gateway == "twocheckout") { _e('Submit and Pay with 2CheckOut', 'pmpro'); } else { _e('Submit and Check Out', 'pmpro'); } } else { _e('Submit and Confirm', 'pmpro');}?> &raquo;" />				
			</span>
		<?php } ?>
...
<script>	
	...
	jQuery('.pmpro_required').parent().find("label").append('<span class="pmpro_asterisk"> *</span>');
